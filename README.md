...
这个题第一眼看过去，竟然把我难住了。我还以为要二分答案，然后就开始疯狂思考如何在规定复杂度内完成一次查询。

不过分析了一下就能发现，由于这个字符串只包含两个字符，因此就必然存在一个连续的段，其中有一种字母全部出现在这个段内，且这个段是0阶平衡的。

找到这个段后，只需要根据查询的q配合段长，再注意一下整个字符串的长度就可以完成这道题。实现的话用前缀和，当然也可以直接二分答案最长的0阶平衡子串。

这个题还是写一下代码吧，正好用一下二分答案。
```cpp
```
